...
| P2-J17 | J-1 表示 | 画面表示の数字の日本語対応（2026-09-26 実機確認で判明）。VEGA の返答は TTS のため数字を言葉で書かせ（`PromptFactory`「Spell out numerals」）、画面表示時に `DisplayNumerals` が算用数字へ戻す設計だが、`DisplayNumerals` は単語境界を「直前が文字でないこと」で判定しており、漢字・かなが続く日本語では数字を検出できない。日本語（`Language.JA`）のときだけ、漢数字の連なり（〇一二三四五六七八九十百千万億兆 と小数の「点」）を検出して、3 桁区切りの算用数字（整数）・小数に戻す。100 未満の整数は既存の方針どおり変換しない（「一番」「十分」などの誤変換防止）。システム名などの既存の算用数字には触れない。読み上げ（TTS）へ渡す文章は変更しない | `app/src/main/java/elite/intel/ui/i18n/DisplayNumerals.java`、`app/src/test/java/elite/intel/ui/i18n/DisplayNumeralsTest.java` | `./gradlew --no-daemon :app:test --tests '*DisplayNumeralsTest' --tests '*LocalizedNumbersTest'` と全体テスト（既知の 10 件以外の失敗 0 件）。例: 「五百十万三千九百五十クレジット」→「5,103,950クレジット」、「十二点六一光年」→「12.61光年」、「一番近い」「十分」は変換しない | **DONE**（2026-09-26、main `7952149`） |
| P2-J18 | J-2 応答 | 日本語応答の数字を算用数字にする（2026-09-26 実機確認で判明）。`PromptFactory.generateAnalysisPrompt` の「Spell out numerals (e.g., twenty-three, not 23).」により、Gemma 4 E4B が日本語で大きな数字を漢数字に書こうとして崩す（例: 17,070,320 → 「一七百七万」）。応答言語（`AiResponseLanguagePolicy.resolveEffectiveAiResponseLanguage`）が `Language.JA` のときだけ、この行を「Write numbers as Arabic numerals with thousands separators (e.g., 17,070,320). Do not write numbers in kanji or kana.」に置き換える。他言語の文言は変更しない。P2-J17 の画面表示変換は、算用数字には触れないため併存できる | `app/src/main/java/elite/intel/ai/brain/commons/PromptFactory.java`、対応するテスト（既存があればそれ、無ければ新規 `PromptFactoryNumeralRuleTest.java`） | 新規／既存テスト（JA のとき算用数字の規則・EN のとき従来の Spell out の規則が入ること）と全体テスト（既知の 10 件以外の失敗 0 件）。実機で日本語の返答の数字が算用数字・3 桁区切りになること。**DONE**（2026-09-26、main `8c8c3c8`。実機再確認待ち） |
