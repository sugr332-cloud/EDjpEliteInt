...
| **Strict Schema 適合率** | **50 / 50 (100.0%)** | 50 / 50 (100.0%) | **50 / 50 (100.0%)** | 同等 |
...
* **プロセス安定性**:
  - 成功数: 50 / 50、Strict Schema 適合数: 50 / 50 (100.0%)
  - タイムアウト・エラーはゼロ件
...
| **Strict Schema 適合数** | 1 / 50 (2.0%) | **50 / 50 (100.0%)** | **+49 件 (100% 達成)** |
...
* **考察**:
  ...
  - `--effort low` を明示的に指定したことで、推論が 15 秒以内に安定して収束し、100% のスキーマ準拠率とゼロタイムアウトが達成されました。
...
本ベンチマークは、通常のテストスイート (`./gradlew test`) から `@Tag("benchmark")` により除外されており、CI や通常ビルドに負荷を与えません。将来のモデルアップデートや CLI 仕様変更時に同一条件で再測定を行う場合は、以下のコマンドを実行します。
